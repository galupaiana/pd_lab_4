...
class AppWindow(QtWidgets.QWidget):
    ...
    def select_folder(self):
        ...
        try:
            if not (
                os.path.exists(os.path.join(selected_folder, "positive"))
                and os.path.exists(os.path.join(selected_folder, "negative"))
            ):
            ...
            QtWidgets.QMessageBox.information(
                self, "Success", "Folder successfully selected!"
            )
    ...
    def create_annotation_file(self):
        if not self.folderpath:
            QtWidgets.QMessageBox.warning(
                self, "Warning", "Please select a dataset folder first."
            )
            return
        ...
        if save_path:
            try:
                ...
                QtWidgets.QMessageBox.information(
                    self, "Success", "Annotation file successfully created!"
                )
            except Exception as e:
                QtWidgets.QMessageBox.critical(
                    self, "Error", f"An error occurred: {str(e)}"
    ...
    def perform_copy_and_rename(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.information(
                self, "Success", "Files successfully copied and renamed!"
            )
        except Exception as e:
            QtWidgets.QMessageBox.critical(
                self, "Error", f"An error occurred: {str(e)}"
    ...
    def perform_randomize_and_copy(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.information(
                self, "Success", "Files successfully randomized and copied!"
            )
        except Exception as e:
            QtWidgets.QMessageBox.critical(
                self, "Error", f"An error occurred: {str(e)}"
    ...
    def show_next_negative(self):
        try:
            ...
            if next_file:
                with open(
                    next_file, "r", encoding="utf-8"
                ) as f:  # Указана кодировка 'cp1251'
                    content = f.read()
                    self.text_display.setPlainText(content)
            else:
